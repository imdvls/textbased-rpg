...
function Scene({ scene, onSelectOption, onReset, encounterActive, encounterOutcome }) {
  ...
  // Format scene text with paragraph breaks
  const formattedText = scene.text.split('. ').join('.\n\n').replace(/\n\n$/, '');

  // Add atmospheric elements based on scene content
  const hasBlood = scene.text.toLowerCase().includes('blood');
  const hasDarkness = scene.text.toLowerCase().includes('dark');
  const hasWhispers = scene.text.toLowerCase().includes('whisper');
  const hasCold = scene.text.toLowerCase().includes('cold');
